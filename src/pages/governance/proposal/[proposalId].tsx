...
export default function Proposal() {
  ...
  if (data) {
    ;[data].forEach((proposal) => {
      ...
      proposal.status = currentBlock > proposal.endBlock ? i18n._(t`closed`) : i18n._(t`active`)
      proposal.statusText =
        currentBlock > proposal.endBlock ? i18n._(t`${expireTime} ago`) : i18n._(t`${expireTime} left`)
  ...
  return (
    <Container id="vote-page" className="py-4 space-y-6 md:py-8 lg:py-12" maxWidth="2xl">
      <Head>
        <title key="title">Vote | DOGEBARK</title>
        <meta key="description" name="description" content="Vote using xMIST on community created proposals." />
      ...
      <div className="p-4 mb-3 space-y-3">
        ...
        <Typography component="h1" variant="h2">
          {proposal && <div>{proposal?.title}</div>}
        </Typography>
      </div>
      <div className="p-4 space-y-4 rounded bg-dark-900">
        <div className="grid grid-flow-row gap-3">
          {!proposal && <div>{i18n._(t`Proposal ${proposalId} not found`)}</div>}
          {proposal && (
            <>
              <div className="flex flex-col w-full p-4 border-t-0 rounded bg-dark-800">
                <div>
                  <span className="flex items-baseline float-right text-xs flex-nowrap">
                    {i18n._(t`Voting proposal`)}{' '}
                    <Copy className="ml-1 text-primary" toCopy={proposal?.proposalId}>
                      {shortenString(proposal?.proposalId, 10)}
                    </Copy>
                  </span>
                </div>
                ...
                <div className="">
                  <span
                    className={`pl-2 pr-2 text-white rounded border border-white ${
                      proposal?.status === i18n._(t`active`) ? 'bg-[#2edd7d]' : 'bg-[#dd3a2e]'
                    }`}
                  >
                    {proposal?.status}
                  </span>
                  <span className="float-right">
                    {i18n._(t`ends at block ${proposal?.endBlock} (${proposal?.statusText})`)}
                  </span>
                </div>
                ...
                <div className="gap-4">
                  ...
                  <span className="float-left mt-2">
                    Your voting power: {formatXmist(proposal.userVotingPower)} xMIST
                  </span>
              ...
              <div className="flex flex-col w-full p-4 border-t-0 rounded bg-dark-800">
                <span className="flex flex-nowrap">
                  Select a voting option and click on <span className="ml-1 font-bold">Vote</span>
                </span>
                {proposal?.options.map((option, index) => (
                ...
                <div className="flex self-center w-64 mt-4">
                  {!account ? (
                    ...
                    <Button onClick={vote} color="gradient" variant="filled">
                      {i18n._(t`Vote`)}
                    </Button>
                  )}
                </div>
              ...
              <div className="flex flex-col w-full p-4 border-t-0 rounded bg-dark-800">
                {proposal?.votes.length === 0 && <div className="text-base font-bold">{i18n._(t`No votes yet`)}</div>}
                  ...
                  <div key={vote.address} className="grid grid-cols-10">
                    ...
                    <div className="flex flex-col items-end col-span-2">
                      <span className="flex items-baseline flex-nowrap">
                        {formatXmist(vote.amount)} <span className="hidden pl-1 text-xs md:flex">xMIST</span>
                      </span>
                    </div>
                  </div>
                ))}
