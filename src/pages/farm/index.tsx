...
export default function Farm(): JSX.Element {
  ...
  if (chainId === 2000) {
    ...
    if (dogeUSDCPool.reserves) {
      bchPriceUSD =
        Number.parseFloat(dogeUSDCPool.reserves[0].toFixed(USDT.decimals)) /
        Number.parseFloat(dogeUSDCPool.reserves[1].toFixed())
    }

    const dogmoneyUSDCPool = farms.find((v) => v.pair === '0x90104885C69b980cB3C9D351671F0F06E016aB48').pool
    if (dogmoneyUSDCPool.reserves) {
      mistPriceUSD =
        Number.parseFloat(dogmoneyUSDCPool.reserves[0].toFixed(USDC.decimals)) /
        Number.parseFloat(dogmoneyUSDCPool.reserves[1].toFixed())
    }
  }
  ...
  return (
    ...
    >
      <Head>
        <title>Farm | DOGEBARK</title>
        <meta key="description" name="description" content="Farm DOGEBARK" />
      </Head>
