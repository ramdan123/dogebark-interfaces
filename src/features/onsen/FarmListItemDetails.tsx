...
const FarmListItem = ({ farm }) => {
  ...
  try {
    token0Amount = CurrencyAmount.fromRawAmount(farm.pair.token0, JSBI.BigInt(token0Reserve))
      .multiply(Math.round(poolFraction * chefPart * 1e10) || 0)
      .divide(1e10)
  } catch (e) {
    console.error(e)
  }
  ...
  try {
    token1Amount = CurrencyAmount.fromRawAmount(farm.pair.token1, JSBI.BigInt(token1Reserve))
      .multiply(Math.round(poolFraction * chefPart * 1e10) || 0)
      .divide(1e10)
  } catch (e) {
    console.error(e)
      ...
      <Disclosure.Panel className="flex flex-col w-full border-t-0 rounded rounded-t-none bg-dark-800" static>
          ...
          <div className="col-span-2 text-center md:col-span-1">
            {account && (
              <div>
                ...
                {!isMobile && <div className="pr-4 mb-2 text-sm text-right cursor-pointer text-secondary">&nbsp;</div>}
              </div>
            )}
          ...
          <div className="col-span-2 text-center md:col-span-1">
            {account && (
              <div>
                <div className="pr-4 mb-2 text-sm text-right cursor-pointer text-secondary">
                  ...
                  {amount && farm.pool
                    ? `(${formatPercent(
                        Math.min((Number.parseFloat(amount?.toFixed()) / farm.chefBalance) * 100, 100)
                      ).toString()} ` +
                      i18n._(t`of pool`) +
                      `)`
                    : ''}
                ...
                <div className="pr-4 mb-2 text-sm text-right cursor-pointer text-secondary">
                  {Number.parseFloat(token0Amount.toFixed(token0Amount.currency.decimals)).toFixed(
                    token0Amount.currency.decimals > 2 ? 2 : undefined
                  )}{' '}
                  {token0Name}+
                  {Number.parseFloat(token1Amount.toFixed(token1Amount.currency.decimals)).toFixed(
                    token1Amount.currency.decimals > 2 ? 2 : undefined
                  )}{' '}
                  {token1Name}({formatNumber(poolFraction * farm.tvl, true)})
                </div>
              </div>
            )}
            <div className="relative flex items-center w-full mb-4">
            ...
            <Button
              ...
              disabled={
                pendingTx ||
                !typedWithdrawValue ||
                (amount && (amount.lessThan(typedWithdrawValue) || amount.equalTo(0)))
              }
              onClick={async () => {
                setPendingTx(true)
                try {
        ...
        {pendingSushi && pendingSushi.greaterThan(ZERO) && (
          <div className="px-4 pb-4">
            ...
            >
              {i18n._(t`Harvest`) +
                ` ${formatNumber(pendingSushi.toFixed(18))} DOGEBARK ${
